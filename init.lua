...
local S = core.get_translator and core.get_translator("mtpaint") or function(s)
    return s
end
...
local function perform_fill(set_node, pointed_thing, user, opts)
    ...
    if not clicked or clicked == "ignore" then
        return
    end
...
register_mtpaint_point_tool({
    ...
    action = function(_, pointed_thing, user, right_click)
        ...
        if not node or node == "ignore" then
            return
        end
        ...
        if
            oldnode ~= ""
            and not inv:contains_item("main", oldnode)
            and inv:room_for_item("main", ItemStack(oldnode))
        then
